...
export async function resolveRecipients(
  ...
  if (input.recipientType === 'user') {
    if (input.contactIds.length > 0) {
      const contacts = await db.selectFrom('contacts').selectAll().where('id', 'in', input.contactIds).execute()
    ...
    const groups = await db.selectFrom('groups').selectAll().where('id', 'in', input.groupIds).execute()
...
// wherever a contact doesn't already have one on file.
